...
def main():
    ...
    if statistics == 'influxdb':
        df['l_int'] = df.apply(lambda row: snmp_get.get_ifIndex_IP(
            row['source'], row['l_ip']), axis=1)
        df['util'] = df.apply(lambda row: snmp_get.get_uti_ifIndex(
            row['source'], row['l_int'], 0), axis=1)
        df['capacity'] = df.apply(lambda row: snmp_get.get_capacity_ifIndex(
            row['source'], row['l_int']), axis=1)
        df['errors'] = df.apply(lambda row: snmp_get.get_errors_ifIndex(
            row['source'], row['l_int'], 0), axis=1)
    else:
        df['l_int'] = 3
        df['util'] = 200
