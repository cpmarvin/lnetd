...
def get_util_router(lst):
    ...
    max_value = df_merged['bps'].max() / 1000 #1000000000 #demo uses 1000 real is 1000000000
    ...
    max_value_inbound = df_merged_inbound['bps'].max() / 1000 #1000000000 #demo uses 1000 real is 1000000000
...
def get_capacity_router(lst):
    ...
    return capacity / 100 #1000 #demo uses 100 real data is 1000 

def get_max_util(hostname,interface,start):
        if interface == 0:
                    return -1
        int(start)
        queryurl = '''SELECT max(non_negative_derivative) from 
