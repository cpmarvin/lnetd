function lnet_d3js(web_ip,result,type){
  ...
  var svg = d3.select("#topology").append("svg")
                    .call(zoom)
		    .call(zoom.transform, d3.zoomIdentity.translate(200, 80).scale(0.8))
                    .attr("id","main_svg")
                    .attr("width", width)
                    .attr("height", height)
...
const mouseOutFunction = function () {
  ...
  nodeEnter.append("circle") // enter the circle on the g
        .attr("r", function (d) { if (node_weight(d) > 2){ return 10 } else { return 30 } })
        .attr('class', 'circle')
        //.attr('fill', 'red');
  nodeEnter.append("image").attr("class","image")
        .attr("xlink:href", function (d) { if (node_weight(d) > 2){ return "/static/images/spine.svg"  }
					   else { return "/static/images/leaf.svg" }
						})
        .attr("x", function (d) { if (node_weight(d) > 2) { return "-152px" } else { return "-112px" } })
        .attr("y", function (d) { if (node_weight(d) > 2) { return "-152px" } else { return "-128px" } })
        .attr("width", function (d) { if (node_weight(d) > 2) { return "290px" } else { return "260px" } })
        .attr("height", function (d) { if (node_weight(d) > 2) { return "290px" } else { return "260px" } })
