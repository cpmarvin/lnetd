//zoom
var zoom_layer1 = d3.zoom()
  .scaleExtent([0.1, 900])
  //.translateExtent([[150, 150],[150, 150]])
  .on('zoom', zoomFnLayer1);
...
function zoomFnLayer1() {
  d3.select("#main_svg_layer1").select('g')
...
function data_prepare(grapheDatasInitial,re_source,re_target,node_position) {
  ...
  //console.log('this is the result for returnX inside:',result)
  return result
...
//console.log('this is the result for returnY inside:',result)
    return result
}
...
//end on node click 
//BEGIN on latency click
        function on_node_click_latency(web_ip,d) {
            if (selectedNodes.length <2) {
                d3.selectAll(".link").attr("stroke-width",1)
                d3.selectAll(".link").attr("stroke-dasharray",null)
                alert_text = 'Selected:' + d.name
...
//END on latency click 

//BEGIN UTIL LATENCY
        function get_util_latency(d,start) {
            var legend = [
...
//used in spf calculation for latency
function check_link_latency(id) {
			id = id
