...
def topology_errors():
    ...
    # print isis_links
    return render_template(
        "topology_errors.html", values=isis_links, node_position=node_position
    )
...
def topology_latency():
    ...
    node_position = node_position.to_dict(orient="records")
    df = pd.read_sql(
        db.session.query(Links_latency).filter(Links_latency.index >= 0).statement,
        db.session.bind,
    )
    isis_links = df.to_dict(orient="records")
    return render_template(
        "topology_latency.html", values=isis_links, node_position=node_position
    )
...
def model_demand():
    ...
    return render_template(
        ...
        netflow_demands=netflow_demands,
        node_position=node_position,
        columns_demands=columns_demands,
    )
...
def topology_time():
    ...
    isis_links = df.to_dict(orient="records")
    return render_template(
        "topology_time.html", values=isis_links, node_position=node_position
    )
...
def model_demand_lsp():
    ...
    return render_template(
        "model_demand_lsp.html",
        values=isis_links,
        columns=columns,
        router_name=router_name,
        netflow_demands=netflow_demands,
        node_position=node_position,
        columns_lsp=columns_lsp,
        columns_demands=columns_demands,
    )
...
def topology_netbox():
    ...
    return render_template(
        "topology_netbox.html", values=isis_links, node_position=node_position
    )
