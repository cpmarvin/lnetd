...
def get_demand_netflow():
    try:
        ...
        df_netflow['bps'] = df_netflow.apply(
            lambda row: row['bytes'] * 8 / 300, axis=1)
        df_netflow['source'] = df_netflow.apply(
            lambda row: map_ip_to_name(routers_dict, row['peer_ip_src']), axis=1)
        df_netflow['target'] = df_netflow.apply(
            lambda row: map_ip_to_name(routers_dict, row['peer_ip_dst']), axis=1)
        df_netflow = df_netflow[df_netflow['target'].str.contains(
            "-1") == False]
        df_netflow = df_netflow.drop(
            ['peer_ip_src', 'peer_ip_dst', 'bytes'], axis=1)
