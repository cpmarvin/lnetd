...
from database import db
from objects_v2.models import Routers, Inventory_cards, Inventory_interfaces


@blueprint.route('/inventory_network')
@login_required
def inventory_network():
    cards = Inventory_cards.query.all()
    interfaces = Inventory_interfaces.query.all()
    counters = { 'cards':len(cards),'interfaces':len(interfaces) }
    interfaces_status ={ "interfaces_status": Counter(k.interface_status for k in interfaces) }
    return render_template('inventory_network.html', cards=cards,interfaces=interfaces, counters=counters,interfaces_status=interfaces_status)
